...
$MAX_PER_RUN = 300; // garde-fou anti-timeout (relancer pour la suite si liste plus grande)
$flash = null;
    ...
    if (!verify_csrf()) {
        ...
        if ($subject === '' || $bodyHtml === '') {
            ...
            if ($test !== '') {
                ...
                $flash = $ok ? ['ok', 'E-mail de test envoyé à ' . e($test) . '.']
                ...
                try {
                    ...
                    $sql .= " ORDER BY id ASC LIMIT " . (int) $MAX_PER_RUN;
                    ...
                    $flash = ['ok', "Campagne envoyée : {$sent} e-mail(s) partis" . ($fail ? ", {$fail} échec(s)" : '')
                        . '.' . (count($emails) >= $MAX_PER_RUN ? " (lot de {$MAX_PER_RUN} — relancez pour la suite)" : '')];
                } catch (Throwable $e) {
...
<?php if ($flash): ?><div class="alert alert--<?= e($flash[0]) ?>"><?= $flash[1] ?></div><?php endif; ?>
...
<form method="post" class="form glass" style="max-width:760px;padding:1.6rem;border-radius:18px">
    ...
    <p class="muted" style="font-size:.82rem">Astuce : envoie-toi d'abord un <strong>test</strong>, vérifie le rendu, puis laisse ce champ vide pour envoyer à toute la liste. Envoi par lot de <?= (int) $MAX_PER_RUN ?> max.</p>

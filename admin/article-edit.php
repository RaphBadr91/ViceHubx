...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        try {
            ...
            $up = db()->prepare(
                'UPDATE articles SET category_id=?, lang=?, title=?, slug=?, excerpt=?, body=?, badge=?, image=?, status=?, published_at=?
                 WHERE id=?'
            );
            $up->execute([$catId, $lang, $title, $slug, $excerpt, $body, $badge, $image, $status, $pub, $id]);
